...
$$
h_n \approx s_n + (n-1)*(s_n - s^*_{n-1}) , \tag{10}
$$
...
> 其中$s^*$是下一期（年）的收益率（债券出售期）。现在我们将等式（10）代入式（9），注意到$ns_n = s_n + (n-1)*s_n$：

$$
f_{1,n} \approx \frac{h_n + (n-1)*s^*_{n-1} - s_1}{n-1} = s^*_{n-1} + \frac{h_n - s_1}{n-1} . \tag{11}
$$
...
$$
f_{1,n} \approx E(s^*_{n-1}) + \frac{BRP_n}{n-1} , \tag{12}
$$
