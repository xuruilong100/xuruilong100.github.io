...
在之前的博客[《在 R 中估计 GARCH 参数存在的问题》](https://www.cnblogs.com/xuruilong100/p/9986088.html)中，Curtis Miller 讨论了 `fGarch` 包和 `tseries` 包估计 GARCH(1, 1) 模型参数的稳定性问题，结果不容乐观。本文承接之前的博客，继续讨论估计参数的稳定性，这次使用的是前文中提到，但没有详尽测试的 `rugarch` 包。
