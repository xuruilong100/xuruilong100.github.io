...
$$
\begin{aligned}
\hat y_{T_m^y + h_m\mid T_m^x} &= \hat \beta_0 + \hat \beta_1 b(L_m;\hat \theta)x_{T_m^x}^{(m)} \\
\hat y_{T_m^y + h_m\mid T_m^x} &= \hat \beta_0 + \sum_{i=1}^N \hat \beta_i b(L_m;\hat \theta_i)x_{i,T_m^x}^{(m)}
\end{aligned}
$$
