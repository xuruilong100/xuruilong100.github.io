...
3. 构造新变量 $y^{\ast}_{t_m} = y_{t_m} - \hat\lambda_0y_{t_m-h_m}​$ 以及 $x^{*(3)}_{t_m + w - h_m} = x^{(3)}_{t_m + w - h_m} - \hat\lambda_0 x^{(3)}_{t_m-( h_m - w)-h_m}​$
4. 对 $y^{\ast}_{t_m} = \beta_0 + \beta_1 b(L_m;\theta)x^{*(3)}_{t_m + w - h_m} + \varepsilon_{t_m}$ 应用 NLS，得到估计 $\hat \theta_1$ 和 新的残差；
